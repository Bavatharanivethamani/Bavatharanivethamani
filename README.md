# 👋 Hi, I'm Bavatharani V

🎓 Bachelor of Information Technology Student (2027)
📍 Chennai, Tamil Nadu, India, 
    CGPA:9.12 (UPTO - 6th SEMESTER)
💡 Aspiring Software Engineer | AI & Machine Learning Enthusiast

---
## Resume Drive link
Google drive Link: https://drive.google.com/file/d/17bcrNJ9nlD0pNAZvGrjaMqMVwq_2d9ed/view

## 🚀 About Me

* 🔭 Currently working on **AI & Deep Learning Projects** 
* 🌱 Learning **Advanced Machine Learning & System Design**
* 💬 Ask me about **Java, Python, SQL, AI/ML**
* ⚡ Strong interest in solving real-world problems using AI

---
## Coding Profile
290+ Problem Solved in Leetcode, 
1400+ Problem solved in Skillrack

---

## 🛠️ Tech Stack

### 💻 Programming Languages

* Java
* Python
* SQL

### 🤖 AI & Machine Learning

* Machine Learning
* Deep Learning
* Natural Language Processing (NLP)
* Computer Vision

### 📊 Data & Visualization

* Pandas
* NumPy
* Matplotlib
* Seaborn
* Power BI (Beginner)
* Tableau (Beginner)

### ⚙️ Tools & Frameworks

* PyTorch
* Git & GitHub

---

## 📌 Featured Projects

### 🧠 Multimodal Emotion Detection

* Built a deep learning model combining **text, audio, and facial features**
* Used **LSTM for audio**, NLP for text, and CNN for image analysis
* Applied **late fusion technique** for improved accuracy

---

### 🛰️ Satellite Image Change Detection

* Developed using **Swin Transformer + ResNet18**
* Detects land changes using satellite imagery
* Generates **binary prediction maps**

---

### 💻 CivicPulse – Smart City Portal

* Developed **Admin Dashboard** for complaint management
* Features: issue assignment, priority tracking, deadline monitoring
* Built using **Java + REST APIs + Angular (Frontend)**

---

## 🏆 Achievements

* 🧠 Solved **200+ problems on LeetCode**
* ⭐ CodeChef **2-Star (Bronze)**
* 🏅 Kaggle **15+ badges**
* 🎯 Hackathon Finalist (IIT Mapothon)
* 🥇 HackerRank Problem Solving – Gold Badge

---

## 💼 Experience

### 📊 Data Analyst Intern – Data Patterns.ai

* Data preprocessing and cleaning using Python
* Automated web data extraction scripts

### 💻 Java Developer Intern – Infosys Springboard

* Built Admin Dashboard for Smart City project
* Integrated frontend with backend APIs

### 🧪 QA Intern – Pazhanam Technologies

* Designed and documented test cases
* Validated application functionality

---


## 📫 Connect With Me

* 🔗 LinkedIn: https://www.linkedin.com/in/bavatharani-v-97224a296/
* 💻 GitHub: https://github.com/Bavatharanivethamani
* 📧 Email: [bavatharanivethamani@gmail.com](mailto:bavatharanivethamani@gmail.com)

---

⭐ *“Consistency and curiosity drive my growth in tech.”*
